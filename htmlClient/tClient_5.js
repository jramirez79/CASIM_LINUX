...
var initLatLong = [41.3893997, 2.1139643];
var initZoom = 17;
var lat;
var long;
var zoom = initZoom;
...
function send3DElement (modViewMat){
    ...
    for(i=0;i<(swap.length*4);++i,++cnta)
  ...
  var regTProg = 0;
	...
	var layer = Tangram.leafletLayer({
        ...
        long = map.getCenter().lng;
				...
				//console.log(" map size object "+sizeMap);
        ...
        map.setView(initLatLong, initZoom);
...
function DrawQuad(gl,texture,flag){
    ...
    if(flag)
		...
		gl.uniform1f(glslProg.zNearUniform,zNearVar);
...
function distance2Ground(zoom){
    //return Geo.metersPerTile(zoom);
}
...
function setCoords(modViewMat){
    ...
    coordMsg[3] = rotAng;
		...
		//console.log("ModViewMAt "+ modViewMat);
...
function toMercatorCoordinates(){
		...
		var latLong = [lat,long];
		//var resLL = Geo.latLngToMeters(latLong);
		rectZ = resLL[0]-initCartCoords[0];
		rectX= resLL[1]-initCartCoords[1];

		//console.log("latLong Geo: "+resLL);
		//console.log("rectX "+initCartCoords[0]+" ;;;; rectZ "+initCartCoords[1]);
		//console.log("rectX "+rectX+" ;;;; rectZ "+rectZ);
}
