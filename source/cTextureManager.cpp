										  ...
										  unsigned int		envMode		)
{
	...
	else
	{
		...
		log_manager->log( LogManager::LERROR, "While loading texture file: \"%s\"!", base_name.c_str() );
...
//<-RETURN_IT_IF_ALREADY_EXISTS
	...
	for( unsigned int i = 0; i < filenames.size(); i++ )
	{
		...
		ilGenImages( 1, &img_id );
		ilBindImage( img_id );
		success = ilLoadImage( filenames[i].c_str() );
		if( success == IL_TRUE )
		{
			if( flip ) iluFlipImage();
			width	= ilGetInteger( IL_IMAGE_WIDTH );
		...
		{
			global_success = false;
			log_manager->log( LogManager::LERROR, "While loading file \"%s\"!", (char*)filenames[i].c_str() );
			ILenum Error;
...
unsigned int TextureManager::loadRawTexture		( string&			filename,
												  ...
												  bool				do_log )
{
	//TODO finish loadRawTexture method
...
void TextureManager::updateTexture( unsigned int	tex_id,
									float*			data,
		  	  	  	  	  	  	  	GLint			parameter,
		  	  	  	  	  	  	  	GLuint			height,
		  	  	  	  	  	  	  	GLuint			width,
...
{
	glBindTexture( texTarget, tex_id );
		glTexSubImage2D (texTarget, 0, 0, 0, width, height, format, GL_FLOAT, data);
	glBindTexture( texTarget, 0 );
}
//
//=======================================================================================
